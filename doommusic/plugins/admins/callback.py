...
from doommusic import YouTube, app
from doommusic.core.call import YASH
from doommusic.misc import SUDOERS, db
from doommusic.utils.database import (
    get_active_chats,
    get_autoplay,
    get_lang,
    get_upvote_count,
...
from doommusic.utils.database import get_assistant
from doommusic.utils.decorators.language import languageCB
from doommusic.utils.formatters import seconds_to_min
from doommusic.utils.inline import close_markup, stream_markup, stream_markup_timer
from doommusic.utils.stream.autoclear import auto_clean
from doommusic.utils.thumbnails import get_thumb
from config import (
    BANNED_USERS,
    SOUNCLOUD_IMG_URL,
    STREAM_IMG_URL,
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Autoplay":
        from doommusic.utils.database import get_autoplay_owner, set_autoplay_owner
        ...
        await YASH.pause_stream(chat_id)
        ...
        await YASH.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        if command == "Skip":
            ...
            try:
                ...
                if not check:
                    ...
                    try:
                        return await YASH.stop_stream(chat_id)
                    except:
                        return
            except:
                try:
        ...
        if "live_" in queued:
            ...
            try:
                ...
                image = None
            try:
                await YASH.skip_stream(chat_id, link, video=status, image=image)
            except:
            ...
            try:
                ...
                image = None
            try:
                await YASH.skip_stream(chat_id, file_path, video=status, image=image)
            except:
                return await mystic.edit_text(_["call_6"])
            button = stream_markup(_, chat_id)
            img = await get_thumb(videoid)
            ...
            try:
                await YASH.skip_stream(chat_id, videoid, video=status)
            except:
                return await CallbackQuery.message.reply_text(_["call_6"])
            button = stream_markup(_, chat_id)
            run = await CallbackQuery.message.reply_photo(
            ...
            if videoid == "telegram":
                ...
                try:
                    ...
                    image = None
            try:
                await YASH.skip_stream(chat_id, queued, video=status, image=image)
            except:
                return await CallbackQuery.message.reply_text(_["call_6"])
            if videoid == "telegram":
                button = stream_markup(_, chat_id)
